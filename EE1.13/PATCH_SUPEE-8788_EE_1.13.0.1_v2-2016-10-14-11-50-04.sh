...
SUPEE-8788 | EE_1.13.0.1 | v2 | df25d2ad60f6bda7c1047ee5c9d3400a492c5cee | Thu Oct 13 15:27:53 2016 -0700 | ba8d364f2b177842122100d099e5e1df8745995c
...
index 411206d..27284c2 100644
--- app/code/core/Enterprise/PageCache/Model/Processor.php
+++ app/code/core/Enterprise/PageCache/Model/Processor.php
@@ -701,7 +701,13 @@ class Enterprise_PageCache_Model_Processor
          * Define request URI
          */
         if ($uri) {
...
@@ -367,7 +370,7 @@ class Mage_Wishlist_Helper_Data extends Mage_Core_Helper_Abstract
     }
...
diff --git dev/tests/unit/testsuite/Mage/Catalog/controllers/Product/CompareControllerTest.php dev/tests/unit/testsuite/Mage/Catalog/controllers/Product/CompareControllerTest.php
new file mode 100644
index 0000000..bc124ea
--- /dev/null
+++ dev/tests/unit/testsuite/Mage/Catalog/controllers/Product/CompareControllerTest.php
@@ -0,0 +1,64 @@
+<?php
+/**
+ * Magento
...
+
+/**
+ * Test for Mage catalog Product Compare controller
+ *
+ * @category   Mage
+ * @package    Mage_Checkout
+ * @author     Magento Core Team <core@magentocommerce.com>
+ */
+class Mage_Catalog_Product_CompareControllerTest extends PHPUnit_Framework_TestCase
+{
+    /**
...
+     * @var string
+     */
+    protected $_className = 'Mage_Catalog_Product_CompareController';
+
+    /**
+     * Test Mage_Catalog_Product_CompareController::addAction() that form_key param is checked
+     */
+    public function testAddAction()
+    {
+        $controllerMock = $this->getMockBuilder($this->_className)
+            ->disableOriginalConstructor()
+            ->setMethods(array('_validateFormKey', '_redirectReferer'))
+            ->getMock();
+        $controllerMock->expects($this->once())
+            ->method('_validateFormKey')
+            ->will($this->returnValue(false));
+        $controllerMock->expects($this->once())
+            ->method('_redirectReferer');
+
+        $result = $controllerMock->addAction();
+
+        $this->assertNull($result);
+    }
+}
diff --git dev/tests/unit/testsuite/Mage/Checkout/controllers/CartControllerTest.php dev/tests/unit/testsuite/Mage/Checkout/controllers/CartControllerTest.php
index c224d0f..95a3d7b 100755
--- dev/tests/unit/testsuite/Mage/Checkout/controllers/CartControllerTest.php
+++ dev/tests/unit/testsuite/Mage/Checkout/controllers/CartControllerTest.php
@@ -117,4 +117,26 @@ class Mage_Checkout_CartControllerTest extends PHPUnit_Framework_TestCase
             ->method('_goBack');
         $result = $controllerMock->addgroupAction();
     }
+
+    /**
+     * Test Mage_Checkout_CartController::updatePostAction() that form_key param is checked
+     */
+    public function testUpdatePostAction()
+    {
+        $controllerMock = $this->getMockBuilder($this->_className)
+            ->disableOriginalConstructor()
+            ->setMethods(array('_validateFormKey', '_redirect'))
+            ->getMock();
+        $controllerMock->expects($this->once())
+            ->method('_validateFormKey')
+            ->will($this->returnValue(false));
...
+
+        $result = $controllerMock->updatePostAction();
+
+        $this->assertNull($result);
+    }
 }
diff --git dev/tests/unit/testsuite/Mage/Customer/controllers/AccountControllerTest.php dev/tests/unit/testsuite/Mage/Customer/controllers/AccountControllerTest.php
index d9028bd..9385863 100755
--- dev/tests/unit/testsuite/Mage/Customer/controllers/AccountControllerTest.php
+++ dev/tests/unit/testsuite/Mage/Customer/controllers/AccountControllerTest.php
@@ -373,9 +373,11 @@ class Mage_Customer_AccountControllerTest extends PHPUnit_Framework_TestCase
      */
     public function testCreatePostRequiredConfirmation()
     {
-        $successMessage = 'Account confirmation is required. Please, check your email for the confirmation link. To resend the confirmation email please <a href="%s">click here</a>.';
+        $successMessage = 'Account confirmation is required. Please, check your email for the confirmation link.' .
+            ' To resend the confirmation email please <a href="%s">click here</a>.';
         $url = "http://some_url.com";
-        $translatedMessage = 'Account confirmation is required. Please, check your email for the confirmation link. To resend the confirmation email please <a href="' . $url .'">click here</a>.';
+        $translatedMessage = 'Account confirmation is required. Please, check your email for the confirmation link.' .
+            ' To resend the confirmation email please <a href="' . $url .'">click here</a>.';
         list(, $customer, $sessionMock, $requestMock, ,) = $this->_prepareObjects();
 
         $objectMethods = array(
@@ -1327,7 +1329,9 @@ class Mage_Customer_AccountControllerTest extends PHPUnit_Framework_TestCase
             ->will($this->returnValue($customerData));
         $sessionMock->expects($this->once())
             ->method('addError')
-            ->with('There is already an account with this email address. If you are sure that it is your email address, <a href=' . "'" . $url . "'" . '>click here</a> to get your password and access your account.');
+            ->with('There is already an account with this email address.' .
+                ' If you are sure that it is your email address, <a href=' . "'" . $url . "'" . '>click here</a>' .
+                ' to get your password and access your account.');
 
         $requestMock->expects($this->any())
             ->method('isPost')
@@ -1354,8 +1358,12 @@ class Mage_Customer_AccountControllerTest extends PHPUnit_Framework_TestCase
             ->will($this->returnValue($url));
         $objectMock->expects($this->once())
             ->method('__')
-            ->with($this->equalTo('There is already an account with this email address. If you are sure that it is your email address, <a href="%s">click here</a> to get your password and access your account.'), $this->equalTo($url))
-            ->will($this->returnValue('There is already an account with this email address. If you are sure that it is your email address, <a href=' . "'" . $url . "'" . '>click here</a> to get your password and access your account.'));
+            ->with($this->equalTo('There is already an account with this email address.' .
+                ' If you are sure that it is your email address, <a href="%s">click here</a>' .
+                ' to get your password and access your account.'), $this->equalTo($url))
+            ->will($this->returnValue('There is already an account with this email address.' .
+                ' If you are sure that it is your email address, <a href=' . "'" . $url . "'" . '>' .
+                'click here</a> to get your password and access your account.'));
         $objectMock->expects($this->at(7))
             ->method('_getUrl')
             ->with($this->equalTo('*/*/create'), $this->equalTo(array('_secure' => true)))
@@ -1432,6 +1440,28 @@ class Mage_Customer_AccountControllerTest extends PHPUnit_Framework_TestCase
 
         $objectMock->createPostAction();
     }
+
+    /**
+     * Test Mage_Customer_AccountController::loginPostAction() that form_key param is checked
+     */
...
+            ->will($this->returnSelf());
+
+        $result = $controllerMock->loginPostAction();
+
+        $this->assertNull($result);
+    }
 }
 /**
  * Fixture class for Mage_Customer_AccountController
diff --git dev/tests/unit/testsuite/Mage/Review/controllers/ProductControllerTest.php dev/tests/unit/testsuite/Mage/Review/controllers/ProductControllerTest.php
new file mode 100644
index 0000000..49234a8
--- /dev/null
+++ dev/tests/unit/testsuite/Mage/Review/controllers/ProductControllerTest.php
@@ -0,0 +1,64 @@
+<?php
+/**
+ * Magento
...
+ * @author     Magento Core Team <core@magentocommerce.com>
+ */
+class Mage_Review_ProductControllerTest extends PHPUnit_Framework_TestCase
+{
+    /**
+     * Class name that is being tested
+     *
+     * @var string
+     */
+    protected $_className = 'Mage_Review_ProductController';
+
+    /**
+     * Test Mage_Review_ProductController::postAction() that form_key param is checked
+     */
...
+            ->method('_redirectReferer');
+
+        $result = $controllerMock->postAction();
+
+        $this->assertNull($result);
+    }
+}
diff --git dev/tests/unit/testsuite/Mage/Wishlist/Helper/DataTest.php dev/tests/unit/testsuite/Mage/Wishlist/Helper/DataTest.php
index 227307d..8df6219 100755
--- dev/tests/unit/testsuite/Mage/Wishlist/Helper/DataTest.php
+++ dev/tests/unit/testsuite/Mage/Wishlist/Helper/DataTest.php
@@ -59,11 +59,8 @@ class Mage_Wishlist_Helper_DataTest extends PHPUnit_Framework_TestCase
             ->method('getEntityId')
...
+           'product' => $product->getProductId()
         ...
         $helper = $this->getMock(
@@ -73,11 +70,7 @@ class Mage_Wishlist_Helper_DataTest extends PHPUnit_Framework_TestCase
 
         $sessionMock = $this->getMockBuilder('Mage_Core_Model_Session')
             ->disableOriginalConstructor()
-            ->setMethods(array('getFormKey'))
             ->getMock();
-        $sessionMock->expects($this->once())
-            ->method('getFormKey')
-            ->will($this->returnValue($formKey));
         ...
         $storeMock = $this->getMockBuilder('Mage_Core_Model_Store')
             ->disableOriginalConstructor()
@@ -86,7 +79,7 @@ class Mage_Wishlist_Helper_DataTest extends PHPUnit_Framework_TestCase
         $storeMock->expects($this->once())
             ->method('getUrl')
             ->with($this->equalTo('wishlist/index/add'), $this->equalTo($params))
-            ->will($this->returnValue("/wishlist/index/add/product/1/form_key/$formKey/"));
+            ->will($this->returnValue("/wishlist/index/add/product/1/"));
 
         $helper->expects($this->once())
             ->method('_getUrlStore')
@@ -99,7 +92,7 @@ class Mage_Wishlist_Helper_DataTest extends PHPUnit_Framework_TestCase
 
         $url = $helper->getAddUrlWithParams($product, array());
 
-        $this->assertEquals("/wishlist/index/add/product/1/form_key/$formKey/", $url);
+        $this->assertEquals("/wishlist/index/add/product/1/", $url);
     }
 
     /**
diff --git downloader/Maged/Controller.php downloader/Maged/Controller.php
index 80ca2f7..33e6292 100755
